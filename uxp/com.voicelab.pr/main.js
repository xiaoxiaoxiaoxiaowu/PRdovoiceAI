            ...
            <div class="clip-item ${clip.status} ${selectedClips.has(clip.id) ? 'selected' : ''}">
                ...
                <div class="clip-body" style="display:${isCollapsed ? 'none' : 'block'}">
                    <div class="clip-field">
                        <label>音色:</label>
                        <span class="clip-voice-name" onclick="openVoicePanel('${clip.id}')" title="点击切换音色">${escapeHtml(voice?.name || '未选择')}</span>
                    </div>
                    <div class="clip-field">
                        <label>文本:</label>
                    ...
                    <div class="clip-tools">
                        ${voice?.capabilities?.voice_instruction ? `<button class="btn btn-xs btn-insert-instruction" data-clip-id="${clip.id}">[#指令]</button>` : ''}
                        ${is20 ? `<button class="btn btn-xs btn-insert-json20" data-clip-id="${clip.id}">{{2.0}}</button>` : ''}
                    </div>
                    ${is20 ? `
                    <div class="clip-field">
                        <label>模型:</label>
                    ...
                    <div class="clip-field">
                        <label>CoT:</label>
                        <input type="text" class="clip-cot" data-clip-id="${clip.id}" value="${clip.cot_text || ''}" placeholder="思维链引导文本" style="width:100%">
                    </div>
                    ` : ''}
                    ${hasEmotion ? `
                    <div class="clip-field">
                        ...
                        ${is20 ? `<label>强度:</label><input type="range" class="clip-emotion-scale" data-clip-id="${clip.id}" min="1" max="5" value="${clip.emotion_scale}"><span class="scale-val">${clip.emotion_scale}</span>` : ''}
                    </div>
                    ` : ''}
                    ${hasContextTexts ? `
                    <div class="clip-field">
                    ...
                    <div class="clip-field">
                        <label>语速:</label>
                        <input type="range" class="clip-rate" data-clip-id="${clip.id}" min="-50" max="100" value="${clip.speech_rate}">
                        <span class="rate-val">${clip.speech_rate}</span>
                    </div>
                    <div class="clip-field">
                        <label>尾停 (ms):</label>
                        <input type="number" class="clip-silence" data-clip-id="${clip.id}" value="${clip.silence_duration}" min="0" max="30000" style="width:80px">
                    </div>
                    <div class="clip-field">
                        <label>音量:</label>
                        <input type="range" class="clip-loudness" data-clip-id="${clip.id}" min="-50" max="100" value="${clip.loudness_rate}">
                        <span class="loudness-val">${clip.loudness_rate}</span>
                    </div>
                    <div class="clip-field">
                        <label><input type="checkbox" class="clip-subtitle" data-clip-id="${clip.id}" ${clip.enable_subtitle ? 'checked' : ''}> 字幕</label>
                    </div>
                    <div class="clip-actions-row">
                        <button class="btn btn-sm btn-preview" data-clip-id="${clip.id}" ${clip.status !== 'done' ? 'disabled' : ''}>▶ 试听</button>
...
function statusIcon(s) { return { pending: "⏳", generating: "🔄", done: "✅", error: "❌" }[s] || "⏳"; }
...
function bindClipEvents() {
    document.querySelectorAll(".clip-checkbox").forEach(cb => {
        cb.addEventListener("click", (e) => { e.stopPropagation(); toggleSelectClip(cb.dataset.clipId); });
    });
    document.querySelectorAll(".clip-text").forEach(ta => {
        ta.addEventListener("input", () => { const clip = clips.find(c => c.id === ta.dataset.clipId); if (clip) clip.text = ta.value; });
    });
    document.querySelectorAll(".clip-emotion").forEach(sel => {
        sel.addEventListener("change", () => { const clip = clips.find(c => c.id === sel.dataset.clipId); if (clip) clip.emotion = sel.value; });
    });
    document.querySelectorAll(".clip-emotion-scale").forEach(inp => {
        inp.addEventListener("input", () => { const clip = clips.find(c => c.id === inp.dataset.clipId); if (clip) { clip.emotion_scale = parseInt(inp.value); inp.nextElementSibling.textContent = inp.value; } });
    });
    document.querySelectorAll(".clip-expression").forEach(sel => {
        sel.addEventListener("change", () => { const clip = clips.find(c => c.id === sel.dataset.clipId); if (clip) clip.expression = sel.value; });
    });
    document.querySelectorAll(".clip-rate").forEach(inp => {
        inp.addEventListener("input", () => { const clip = clips.find(c => c.id === inp.dataset.clipId); if (clip) { clip.speech_rate = parseInt(inp.value); inp.nextElementSibling.textContent = inp.value; } });
    });
    document.querySelectorAll(".clip-silence").forEach(inp => {
        inp.addEventListener("change", () => { const clip = clips.find(c => c.id === inp.dataset.clipId); if (clip) clip.silence_duration = parseInt(inp.value) || 0; });
    });
    document.querySelectorAll(".clip-loudness").forEach(inp => {
        inp.addEventListener("input", () => { const clip = clips.find(c => c.id === inp.dataset.clipId); if (clip) { clip.loudness_rate = parseInt(inp.value); inp.nextElementSibling.textContent = inp.value; } });
    });
    document.querySelectorAll(".clip-subtitle").forEach(cb => {
        cb.addEventListener("change", () => { const clip = clips.find(c => c.id === cb.dataset.clipId); if (clip) clip.enable_subtitle = cb.checked; });
    });
    document.querySelectorAll(".clip-model").forEach(sel => {
        sel.addEventListener("change", () => { const clip = clips.find(c => c.id === sel.dataset.clipId); if (clip) clip.model = sel.value || null; });
    });
    document.querySelectorAll(".clip-cot").forEach(inp => {
        inp.addEventListener("input", () => { const clip = clips.find(c => c.id === inp.dataset.clipId); if (clip) clip.cot_text = inp.value || null; });
    });
...
function insertInstruction(clipId) {
    ...
    if (instruction) {
        ...
        if (ta) { ta.value = `[#${instruction}]` + ta.value; const clip = clips.find(c => c.id === clipId); if (clip) clip.text = ta.value; }
    }
}
function insertJson20(clipId) {
    ...
    if (ctx) {
        const ta = document.querySelector(`.clip-text[data-clip-id="${clipId}"]`);
        if (ta) { ta.value = `{{"additions":{"context_texts":["${ctx}"]} }}` + ta.value; const clip = clips.find(c => c.id === clipId); if (clip) clip.text = ta.value; }
    }
}
...
function duplicateClip(clipId) {
    ...
    if (clip) {
        ...
        clips.push(newClip); clipCounter++; renderClipList();
    }
}
...
async function generateAll() {
    ...
    document.getElementById("progress-text").textContent = `0/${pending.length}`;
    for (let i = 0; i < pending.length; i++) {
        pending[i].status = "generating";
        renderClipList();
        await generateClip(pending[i]);
        document.getElementById("progress-fill").style.width = `${((i+1)/pending.length)*100}%`;
        document.getElementById("progress-text").textContent = `${i+1}/${pending.length}`;
    }
    generating = false;
}
...
async function generateClip(clip) {
    const payload = {
        id: clip.id, text: clip.text, voice_id: clip.voice_id,
        emotion: clip.emotion, emotion_scale: clip.emotion_scale,
        speech_rate: clip.speech_rate, silence_duration: clip.silence_duration,
        loudness_rate: clip.loudness_rate || 0, bit_rate: clip.bit_rate || null,
        model: clip.model || null, enable_subtitle: clip.enable_subtitle || false,
    ...
    try {
        ...
        if (result.id) {
            clip.status = "done"; clip.audio_url = result.download_url;
            clip.duration_ms = result.duration_ms; clip.file_size = result.size;
            clip.subtitles = result.subtitles || null; clip.usage = result.usage || null;
        } else { clip.status = "error"; }
    } catch (e) { clip.status = "error"; }
    renderClipList();
}
...
async function previewClip(clipId) {
    const clip = clips.find(c => c.id === clipId);
    if (clip && clip.audio_url) { new Audio(`${backendUrl}${clip.audio_url}`).play(); }
}

// ==================== PR 导入（通过消息发送给 host.js） ====================
async function importToPR() {
    ...
    if (doneClips.length === 0) { alert("没有已生成的音频可导入"); return; }

    doneClips.sort((a, b) => (parseInt(a.id.replace(/^clip_/, '')) || 0) - (parseInt(b.id.replace(/^clip_/, '')) || 0));
    ...
    const items = doneClips.map(c => ({ id: c.id, path: `${dir}/${c.id}.mp3`, duration_ms: c.duration_ms || 0 }));

    // 生成唯一请求 ID
    const reqId = "import_" + Date.now();
    // 注册一次性监听
    window.addEventListener("message", function handler(event) {
        if (event.data.id === reqId && event.data.action === "importToTimelineResult") {
            window.removeEventListener("message", handler);
            const result = event.data.result;
            if (result.error) {
                alert("导入失败: " + result.error);
            } else {
                const ok = result.filter(r => r.inserted).length;
                const errs = result.filter(r => r.error);
                let msg = `✅ 已导入 ${ok} 条`;
    ...
    // 发送消息到主进程
    window.postMessage({ id: reqId, action: "importToTimeline", payload: { items, importAtPlayhead } }, "*");
}

// ==================== 文本导入（纯 Webview，无需宿主 API） ====================
async function importSRT() {
    ...
    input.onchange = async (e) => {
        ...
        for (const block of blocks) {
            ...
            let textLine = "";
            if (lines.length >= 3) textLine = lines.slice(2).join(" ");
            else if (lines.length === 2) textLine = /^\d+$/.test(lines[0]) ? lines[1] : lines.join(" ");
            else textLine = lines[0];
            if (textLine.trim()) addClip(textLine, defaultVoice);
        }
    };
    input.click();
}

// ==================== 项目存取（通过消息发送给 host.js） ====================
async function saveProject() {
    const data = {
        clipCounter: clipCounter,
        clips: clips.map(c => ({
            id: c.id, text: c.text, voice_id: c.voice_id, emotion: c.emotion,
            emotion_scale: c.emotion_scale, expression: c.expression,
            speech_rate: c.speech_rate, silence_duration: c.silence_duration,
            loudness_rate: c.loudness_rate || 0, bit_rate: c.bit_rate || null,
            model: c.model || null, enable_subtitle: c.enable_subtitle || false,
            cot_text: c.cot_text || null, _collapsed: c._collapsed, status: "pending",
        })),
    };
    const jsonStr = JSON.stringify(data);
    const reqId = "save_" + Date.now();
    window.addEventListener("message", function handler(event) {
        if (event.data.id === reqId && event.data.action === "saveProjectResult") {
            window.removeEventListener("message", handler);
            const result = event.data.result;
            if (result.cancelled) return;
            if (result.error) alert("保存失败: " + result.error);
            else document.getElementById("project-name").textContent = result.name;
        }
    });
    window.postMessage({ id: reqId, action: "saveProject", payload: jsonStr }, "*");
}

async function openProject() {
    const reqId = "load_" + Date.now();
    window.addEventListener("message", function handler(event) {
        if (event.data.id === reqId && event.data.action === "loadProjectResult") {
            window.removeEventListener("message", handler);
            const result = event.data.result;
            if (result.cancelled) return;
            if (result.error) { alert("打开项目失败: " + result.error); return; }
            const data = JSON.parse(result.content);
            clips = (data.clips || []).map(c => ({ ...c, status: c.status || "pending", audio_url: null, duration_ms: null, file_size: null }));
    ...
    window.postMessage({ id: reqId, action: "loadProject" }, "*");
}

function newProject() {
    if (clips.length === 0 || confirm("确定要新建项目？当前内容将丢失。")) {
        clips = []; clipCounter = 1; selectedClips.clear(); renderClipList();
